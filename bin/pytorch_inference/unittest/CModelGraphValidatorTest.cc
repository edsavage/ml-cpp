...
BOOST_AUTO_TEST_CASE(testMixedForbiddenAndUnrecognised) {
    // When forbidden ops are present, the validator short-circuits and
    // does not report unrecognised ops — we reject immediately.
...
BOOST_AUTO_TEST_CASE(testMaliciousHiddenInSubmodule) {
    // aten::logit (unrecognised) buried three levels deep in nested submodules.
    // The validator must inline through all submodules to find it.
    auto module = ::torch::jit::load("testfiles/malicious_models/malicious_hidden_in_submodule.pt");
    auto result = CModelGraphValidator::validate(module);

    BOOST_REQUIRE(result.s_IsValid == false);
    BOOST_REQUIRE(result.s_ForbiddenOps.empty());
    BOOST_REQUIRE(hasUnrecognisedOp(result, "aten::logit"));
}

BOOST_AUTO_TEST_CASE(testMaliciousConditionalBranch) {
    // An unrecognised op (aten::logit) hidden inside one branch of a
    // conditional. The validator must recurse into prim::If blocks.
...
BOOST_AUTO_TEST_CASE(testMaliciousManyUnrecognisedOps) {
    // A model using several different unrecognised ops (logit, erfc, tan, exp).
    ...
    BOOST_REQUIRE(hasUnrecognisedOp(result, "aten::logit"));
    BOOST_REQUIRE(hasUnrecognisedOp(result, "aten::erfc"));
...
// The graph validator must reject them because aten::as_strided is in
...
// the reference HuggingFace models.  The golden file is generated by
// dev-tools/extract_model_ops/extract_model_ops.py --golden and should
// be regenerated whenever PyTorch is upgraded or the set of supported
// architectures changes.

BOOST_AUTO_TEST_CASE(testAllowlistCoversReferenceModels) {
    std::ifstream file("testfiles/reference_model_ops.json");
    BOOST_REQUIRE_MESSAGE(file.is_open(),
                          "Could not open testfiles/reference_model_ops.json — "
                          "regenerate with: python3 dev-tools/extract_model_ops/"
                          "extract_model_ops.py --golden "
                          "bin/pytorch_inference/unittest/testfiles/reference_model_ops.json");

    std::ostringstream buf;
    buf << file.rdbuf();
    auto root = boost::json::parse(buf.str()).as_object();

    auto& models = root.at("models").as_object();
    BOOST_REQUIRE_MESSAGE(models.size() > 0, "Golden file contains no models");
